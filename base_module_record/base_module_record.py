...
#            name=data.get('name','') or ''.lower()
    ...
    def _get_id(self, cr, uid, model, id):
        ...
        if not dtids:
            # HACK: 11.02.2015 09:10:16: jool1: create ir.model.data if not existing
            vals_dt = {
                       'module': '__export__',
                       'model': model,
                       'res_id': id,
                       'name': model.replace('.','_')+'_'+str(id)
                       }
            new_id = dt.create(cr, uid, vals_dt)
            dtids = dt.search(cr, uid, [('model','=',model), ('res_id','=',id)])
            if not dtids:
                return False, None
#             return False, None
            # END HACK: 11.02.2015 09:10:16: jool1: create ir.model.data if not existing
